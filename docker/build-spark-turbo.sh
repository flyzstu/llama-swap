#!/bin/bash
# Build the combined -spark-turbo variant: stock llama-swap image plus:
# 1. XHToken/llama.cpp llama-server as /app/llama-server-spark
# 2. TheTom/llama-cpp-turboquant llama-server as /app/llama-server-turbo
#
# Usage:
#   GITHUB_TOKEN=... ./docker/build-spark-turbo.sh cuda [push]
#   GITHUB_TOKEN=... ./docker/build-spark-turbo.sh cuda13 true
#   SPARK_COMMIT=<full-sha> TURBO_COMMIT=<full-sha> ./docker/build-spark-turbo.sh cuda true
...
# Pinned commits (override via SPARK_COMMIT and TURBO_COMMIT env vars)
...
for CONTAINER_TYPE in non-root root; do
  ...
  COMBO_TAG="ghcr.io/${LS_REPO}:v${LS_VER}-${ARCH}-${LCPP_TAG}-spark-${SPARK_SHORT}-turbo-${TURBO_SHORT}${SUFFIX}"
  COMBO_LATEST="ghcr.io/${LS_REPO}:${ARCH}-spark-turbo${SUFFIX}"

  # Prefer the base built in this workflow run; fall back to the last daily image
  BASE="$VERSIONED_BASE"
  ...
  log_info "Building $CONTAINER_TYPE $COMBO_TAG (base: $BASE)"
  docker build \
    -f llama-swap-spark-turbo.Containerfile \
    --build-arg BASE="${BASE}" \
    ...
    -t "${COMBO_TAG}" -t "${COMBO_LATEST}" .

  if [ "$PUSH_IMAGES" == "true" ]; then
    docker push "${COMBO_TAG}"
    docker push "${COMBO_LATEST}"
  fi
done
